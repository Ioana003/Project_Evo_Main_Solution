...
namespace Project_Evo_Main_Solution
{
    public class Camera
    {
        ...
        public void Follow(Player target, float zoom)
        {
            position = Matrix.CreateTranslation(-target.spritePosition.X - (target.spriteText.Width / 2), -target.spritePosition.Y - (target.spriteText.Height / 2), 0) * Matrix.CreateScale(zoom);
